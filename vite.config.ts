...
export default defineConfig({
  ...
  server: {
    ...
    proxy: {
      ...
      "/imprint": {
        target: "http://10.10.10.27:8081",
        changeOrigin: true,
        secure: false,
        configure: (proxy, _options) => {
          proxy.on("error", (err, _req, _res) => {
            console.log("imprint proxy error", err);
          });
          proxy.on("proxyReq", (proxyReq, req, _res) => {
            console.log("Sending Imprint Request to the Target:", req.method, req.url);
          });
          proxy.on("proxyRes", (proxyRes, req, _res) => {
            console.log(
              "Received Imprint Response from the Target:",
              proxyRes.statusCode,
              req.url
            );
          });
        },
        rewrite: (path) => path.replace(/^\/imprint/, "/imprint"),
      },
      "/api": {
        target: "http://10.10.10.25:8081",
        changeOrigin: true,
